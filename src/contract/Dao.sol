...
contract Dao is Ownable {
    ...
    function createProposal(
        ...
        proposalsMapping[proposalCount] = Proposal(
            proposalCount,
            _description,
            _image,
            _Type,
    ...
    function addMember(uint _amount) public payable {
        require(
            msg.value == (_amount * Token.getTokenPrice()),
            "Your Amount Value is Low for Transaction"
        );
        require(
    ...
    function upVote(uint _id) public payable {
        ...
        require(
            msg.sender != proposalsMapping[_id].creator,
            "You are a proposal creator so you can not vote"
        ); // proposal creator cannot vote
        require(!hasVoteMapping[_id][msg.sender], "You have already voted"); // member can vote only once
        require(
            proposalsMapping[_id].proposalExpiresAt >= block.timestamp,
            "Voting time ended"
        ); // proposal time expire
    ...
    function getProposalResult(
        ...
        if (totalVotes == 0) {
            ...
            proposalsMapping[_proposalId].upVote >
            proposalsMapping[_proposalId].downVote
        ) {
            ...
            if (proposalsMapping[_proposalId].Type == false) {
                ...
                company.issueCredit(
                    proposalsMapping[_proposalId].creator,
                    carbonCredit
                );
            } else {
                carbonCredit =
                    globalLimit -
                    proposalsMapping[_proposalId].value;
                company.issueCredit(
                    proposalsMapping[_proposalId].creator,
                    carbonCredit
                );
            ...
            uint upVotersStake = proposalsMapping[_proposalId].upVote *
                votingStake;
            memberWithdrawableAmountMapping[
                proposalsMapping[_proposalId].creator
            ] += proposalStake;
            if (upVotersStake > 0) {
                for (
                    uint i = 0;
                    i < proposalsMapping[_proposalId].upVoters.length;
                    i++
                ) {
                    memberWithdrawableAmountMapping[
                        proposalsMapping[_proposalId].upVoters[i]
                    ] += votingStake;
                }
            }
        } else {
            ...
            if (downVotersStake > 0) {
                for (
                    uint i = 0;
                    i < proposalsMapping[_proposalId].downVoters.length;
                    i++
                ) {
                    memberWithdrawableAmountMapping[
                        proposalsMapping[_proposalId].upVoters[i]
