    ...
    Ownable //UserInfo
{
    ...
    function setUser(
        ...
        string memory _companyProfileImage
    ) public {
        //  Check if the company is already registered
        require(
            !isCompaniesAddMapping[msg.sender],
    ...
    function issueCredit(address _address, uint _credits) external {
        ...
        require(
            ...
            "Only DAO address can call this function"
        ); //Dao Address
    ...
    function buyCredit(uint _orderId) public payable {
        ...
        require(
            msg.sender != orderStructMapping[_orderId].seller,
            "Seller cannot buy their own credit"
        );
        require(
            msg.value ==
                (orderStructMapping[_orderId].credit *
                    orderStructMapping[_orderId].price),
            "Not enough amount"
        );
        totalCreditMapping[msg.sender] += orderStructMapping[_orderId].credit;
        creditToSellMapping[
            orderStructMapping[_orderId].seller
        ] -= orderStructMapping[_orderId].credit;
        orderStructMapping[_orderId].isSell = true;
        uint totalPrice = orderStructMapping[_orderId].credit *
            orderStructMapping[_orderId].price;
